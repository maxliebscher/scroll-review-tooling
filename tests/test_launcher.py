...
class LauncherTests(unittest.TestCase):
    ...
    def test_windows_start_here_runs_setup_then_operator(self) -> None:
        ...
        self.assertIn("call RUN_LOCAL_APP.cmd %START_ARGS%", text)
        self.assertIn("call RUN_LOCAL_OPERATOR.cmd %START_ARGS%", text)
        self.assertIn("python --version", text)
        self.assertIn("start_here.log", text)
        self.assertIn("Port 8765", text)
        self.assertIn("Python was not found", text)
        self.assertIn("Primary Windows entry point", text)
        self.assertIn("python.org", text)
        self.assertIn("If you already have the app open", text)
        self.assertIn("If the browser does not open", text)
        self.assertIn("Nothing is uploaded", text)
        self.assertIn("/nopause", text)
        self.assertIn("/noopen", text)
        self.assertIn("demo\\out\\operator.html", text)
        self.assertIn("http://127.0.0.1:8765/", text.lower())
        self.assertNotIn("http://", text.lower().replace("http://127.0.0.1:8765/", ""))
        self.assertNotIn("https://", text.lower())
        self.assertNotIn("powershell", text.lower())

    def test_windows_local_app_launcher_is_loopback_only(self) -> None:
        repo = Path(__file__).resolve().parents[1]
        launcher = repo / "RUN_LOCAL_APP.cmd"
        text = launcher.read_text(encoding="utf-8")
        self.assertIn("python scripts\\operator_server.py", text)
        self.assertIn("127.0.0.1", text)
        self.assertIn("manually open http://127.0.0.1:8765/", text)
        self.assertNotIn("http://", text.lower().replace("http://127.0.0.1:8765/", ""))
        self.assertNotIn("https://", text.lower())
        self.assertNotIn("powershell", text.lower())
