...
- Run `cmd /c START_REVIEW_SESSION.cmd`.
...
- Run `python -m scroll_review_tooling.review_workflow source-catalog --out-json demo/out/source_catalog.json`.
...
- Run `python scripts/check_release.py --include-operator-flow --out-json demo/out/release_check_operator_flow.json --out-md demo/out/release_check_operator_flow.md` before a release-candidate review.
- Run `python scripts/package_check.py --out-json demo/out/package_check.json`
  before preparing any ZIP-style handoff. This is a dry-run check only; it
  must not create an archive.
- Confirm release check includes `package-check: ok` before sharing a ZIP-style
  release candidate.
...
- Confirm public scan-chunk outputs in `demo/out/` are JSON/Markdown summaries
  only; raw chunk bytes must stay in a user-chosen local workspace outside the
  repo.
- Confirm full-volume presets, private credentials, and unsafe workspaces remain
  blocked.
...
- Confirm the package candidate set excludes `demo/out/`, caches, raw scan
  suffixes (`.zarr`, `.tif`, `.tiff`, `.npy`), model/checkpoint files, and
